...
- **인증**
  - `wrap` 계정 추가 (`DUMPIT_WRAP_PASSWORD`) — 일반 권한 + **전체 현황 서브메뉴 전체** 접근
  - `guest`는 기존처럼 제한 메뉴 유지
- **폰트**
  - Pretendard CDN 로드 — Mac/Windows 시스템 폰트 불일치 해소
- **사업장별(회차)**
  - 상환 완료 건에도 **대출만기일·펀드만기일** 다시 표시
- **공정율 현황** (`/admin/progress`)
  - 진행중(active) 랩은 기성 자료가 없어도 목록에 자동 표시 (placeholder)
  - 수정 모드에서 특이사항 입력란을 큰 textarea로 확대
- **브랜드**
  - 사이드바 제목·밑줄: 그라데이션 → 단일 accent 색
